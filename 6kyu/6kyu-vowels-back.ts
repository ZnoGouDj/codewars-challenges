...
function vowelBack(s: string): string {
  let result = [];
...
function moveForward(char: string): string {
  let initialCharacterIndex = ALPHABET.indexOf(char);
  return initialCharacterIndex + 9 >= 26
    ? ALPHABET[initialCharacterIndex + 9 - 26]
    : ALPHABET[initialCharacterIndex + 9];
}

function moveBack(char: string): string {
  let initialCharacterIndex = ALPHABET.indexOf(char);
  return initialCharacterIndex - 5 < 0 ? ALPHABET[initialCharacterIndex - 5 + 26] : ALPHABET[initialCharacterIndex - 5];
}

function checkCODE(char: string): boolean {
  return char === 'c' ? true : char === 'o' ? true : char === 'd' ? true : char === 'e' ? true : false;
}

function moveThisException(char: string): string {
  switch (char) {
    case 'c':
      return 'b';
    case 'o':
