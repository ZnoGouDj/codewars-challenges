...
// Test.assertEquals(sumin(5), 55)
// Test.assertEquals(sumax(8), 372)
// Test.assertEquals(sumsum(8), 576)
// Test.assertEquals(sumin(15), 1240)

let sumin = (n: number): number => (n * n * n) / 3 + (n * n) / 2 + n / 6;
let sumax = (n: number): number => (n * (n + 1) * (4 * n - 1)) / 6;
let sumsum = (n: number): number => n * n * n + n * n;
