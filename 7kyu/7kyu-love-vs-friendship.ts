...
function wordsToMarks(string: string): N {
  let sum = 0;
...
//top
const wordsToMarks1 = (s: string): N => [...s].reduce((res, c) => (res += c.charCodeAt(null) - 96), 0);

//top1
function wordsToMarks2(string: string): N {
  return string.split('').reduce((acc, char) => {
    return acc + char.charCodeAt(null) - 96;
  }, 0);
}
