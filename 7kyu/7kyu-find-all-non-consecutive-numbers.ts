...
function allNonConsecutive(arr: MyType[]): number[] {
  let res = [];
  for (let i = 0; i < arr.length; i++) {
    if (arr[i + 1] !== undefined && Number(arr[i]) - Number(arr[i + 1]) !== -1) {
...
let allNonConsecutive1 = (arr: number[]): number[] =>
  arr.reduce((j, n, i) => (i > 0 && n - 1 !== arr[i - 1] ? [...j, { i, n }] : j), []);
