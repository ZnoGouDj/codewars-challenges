...
function jumpingNumber(n: N): string {
  let numberDivide = String(n).split('');
  for (let i = 0; i < numberDivide.length - 1; i++) {
    if (+numberDivide[i] === +numberDivide[i + 1] + 1 || +numberDivide[i] === +numberDivide[i + 1] - 1) {
...
//top
const jumpingNumber1 = (n: N): string =>
  n
    .toString()
    .split(``)
    .every((e, i, a) => i < 1 || Math.abs(Number(e) - Number(a[i - 1])) === 1)
    ? 'Jumping!!'
    : 'Not!!';
